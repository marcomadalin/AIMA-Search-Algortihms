...
//        String file_name = "debug";
        BufferedWriter w1 = new BufferedWriter(new FileWriter("experiment4.txt"));
...
//        BufferedWriter w3 = new BufferedWriter(new FileWriter(file_name +"-temps.txt"));
        ...
        //System.out.println("Primer apartado fijando el número de paquetes a 100");
        w1.write("Primer apartado fijando el número de paquetes a 100. Variable 'Proporcion'\n");
        for (int prop = 12; prop <= 40; prop += 2) {
            for (int i = 0; i < 10; ++i) {
                long startTime = System.currentTimeMillis();
                Random rand = new Random(System.currentTimeMillis());
                AzamonState a = new AzamonState(100, rand.nextInt(), rand.nextInt(), (double)prop/10);
                a.generate_solution3();
                azamon_local_search(a, 1, 2, 1, w1, null, null, startTime, String.format("%.2f", (double)prop/10));
            }
        }
        w1.write("Segundo apartado fijando la proporcion a 1.2. Variable 'Paquetes'\n");
        for (int packets = 50; packets <= 500; packets += 50) {
            for (int i = 0; i < 10; ++i) {
                long startTime = System.currentTimeMillis();
                Random rand = new Random(System.currentTimeMillis());
                AzamonState a = new AzamonState(packets, rand.nextInt(), rand.nextInt(), 1.2);
                a.generate_solution3();
                azamon_local_search(a, 1, 2, 1, w1, null, null, startTime, packets + "");
            }
...
//        w3.close();
    ...
    private static void azamon_local_search(AzamonState azamon, int h, int op, int alg, BufferedWriter w1, BufferedWriter w2, BufferedWriter w3, long startTime, String s) {
        ...
        try {
            ...
            else search = new SimulatedAnnealingSearch(10000,100, 1, 0.1);
